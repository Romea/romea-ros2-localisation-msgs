...
The package does not implement any algorithm. It provides the common message types used to express localisation observations. Generic finite-state-machine status and event messages are provided by `romea_common_msgs`. The conversion helpers are provided by `romea_localisation_utils`, while localisation filter nodes are provided by packages such as `romea_robot_to_world_localisation_core`, `romea_robot_to_robot_localisation_core` or `romea_robot_to_human_localisation_core`.
